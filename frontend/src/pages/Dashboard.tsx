...
const Dashboard = () => {
  ...
  const sections = {
    ...
    service: ['Profile', 'Password', 'Service Ticket', 'My Services', 'Reviews', 'Schedule', 'Support']
  };
        ...
        <div className="md:hidden p-4 bg-white dark:bg-slate-800 border border-gray-300 dark:border-gray-600 rounded-lg">
          ...
          <div className="relative">
            <button
              ...
              className="w-full p-3 text-left bg-secondary text-white rounded-lg focus:outline-none"
            >
              {selectedSection}
            </button>
            {dropdownOpen && (
              <div className="absolute left-0 w-full mt-2 bg-white dark:bg-slate-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg">
                {roleSections.map(section => (
                  <button
                    key={section}
                    onClick={() => { setSelectedSection(section); setDropdownOpen(false); }}
